...
export function createCoordinate({
  ...
  // 它们还需要传入 transformOptions
  // 它们返回一个由基本变化构成的数组，所以在复合前需要通过 flat 把数组拍平
  // [[transpose, reflect], [transpose, reflect]]
  // -> [transpose, reflect, transpose, reflect]
  const transforms = coordinates.flatMap((coordinate) =>
    coordinate(canvasOptions),
  );
  ...
  // 这里使用了异或：a ^ b，只有当 a 和 b 值不相同的时候才为 true，否则为 false
  output.isTranspose = () =>
